#!/usr/bin/env python3
"""Run last30days topics and send a daily digest to WeCom.
...
def run_last30days(skill_dir: Path, topic: dict[str, str], timeout: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "query": query,
            "status": "failed",
            "error": "last30days returned non-JSON output",
...
def build_markdown(results: list[dict[str, Any]]) -> str:
    ...
    lines = [
        f"**Last30Days Daily Brief - {date_label}**",
        "",
        f"> Topics: {len(results)} | Generated by GitHub Actions",
        "",
    ]
    for result in results:
        ...
        if result["status"] != "ok":
            lines.extend(
                [
                    f"**{name}**",
                    f"> Run failed: {clean_text(result.get('error', 'unknown error'))[:300]}",
                    "",
                ]
            )
            continue
        ...
        lines.append(f"**{name}**")
        lines.append(f"> {result.get('raw_count', 0)} findings" + (f" | {counts}" if counts else ""))
        items = result.get("items", [])
        if not items:
            lines.append("- No high-signal findings returned.")
        for item in items:
            title = item.get("title") or item.get("text") or "Untitled finding"
            meta = " / ".join(
                str(part)
...
def trim_wecom_markdown(markdown: str) -> str:
    ...
    if len(markdown) <= limit:
        return markdown
    suffix = "\n\n> Truncated. Open the workflow artifact for full raw output."
